...
# Arsitektur SRNet (Konsisten dengan sebelumnya)
...
def main():
    model_dir = "models"
    test_base_dir = r"../Dataset Sebelah/Dataset Split RAW BOS/test"
    ...
    for m_file in model_files:
        ...
        for cat in categories:
            ...
            files = [f for f in os.listdir(cat_path) if f.lower().endswith(('.png', '.jpg', '.jpeg'))][:10]
    ...
    # Print Summary Table
    print("\n" + "="*110)
    header = f"{'Model Name':<35} | " + " | ".join([f"{c:<10}" for c in categories])
    print(header)
    print("="*110)
    for m_file, res in all_results.items():
        row = f"{m_file:<35} | " + " | ".join([f"{res[c]:<10}" for c in categories])
        print(row)
    print("="*110)
